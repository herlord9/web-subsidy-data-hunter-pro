...
![image-20251127225208791](E:\bbb-test\data-hunter-pro\README.assets\image-20251127225208791.png)

![image-20251127225903945](https://gitee.com/perfectsky/img/raw/master/img/image-20251127225903945.png)
...
![image-20251127230053920](https://gitee.com/perfectsky/img/raw/master/img/image-20251127230053920.png)

![image-20251127230026867](https://gitee.com/perfectsky/img/raw/master/img/image-20251127230026867.png)
...
![image-20251127230137503](https://gitee.com/perfectsky/img/raw/master/img/image-20251127230137503.png)

### 🔍 关键词快捷搜索

- **JSON 配置关键词** - 关键词列表从 `src/data/keywords.json` 文件加载，支持自定义修改
- **右侧固定选择器** - 在网页右侧显示可滑动的关键词选择器，固定位置，不遮挡内容
- **一键填充搜索** - 点击关键词按钮，自动填充到搜索框并触发搜索
- **自动记录位置** - 每次点击关键词后，自动记录滚动位置，下次打开时自动定位到上次点击的位置
- **页面刷新自动恢复** - 页面刷新后，关键词选择器会自动恢复，滚动位置也会自动恢复
- **支持 27+ 个预定义关键词** - 包含耕地地力、惠农、大豆玉米、稻谷补贴、油菜种植等农业补贴相关关键词

![image-20251127225833321](https://gitee.com/perfectsky/img/raw/master/img/image-20251127225833321.png)
...
2. **使用关键词搜索**（可选）：
   - 点击插件中的"识别搜索框并添加关键词快捷选择"按钮
   - 在网页右侧会出现关键词选择器（从 JSON 文件加载）
   - 点击任意关键词，自动填充到搜索框并触发搜索
   - 点击位置会自动记录，下次打开时自动定位
...
│   └── api.js       # API 配置（环境切换）
├── data/            # 数据文件
│   ├── keywords.json        # 关键词配置文件（JSON 格式）
│   └── shandong_regions.json # 山东省地区数据
├── _locales/        # 多语言文件
└── manifest.json    # 扩展配置
```
...
- ✅ **关键词快捷搜索** - 右侧固定关键词选择器，JSON 配置，点击即搜索，自动记录位置
