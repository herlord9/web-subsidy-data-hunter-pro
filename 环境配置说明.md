...
### 测试环境（默认）
- **环境名称**: 测试环境
- **API 地址**: `http://test.agrolinking.cn:8000`
- **用途**: 日常开发和测试
...
### 生产环境
- **环境名称**: 生产环境
- **API 地址**: `https://api.agrolinking.cn`
- **用途**: 线上正式环境
...
```
🌐 当前环境：测试环境
API: http://test.agrolinking.cn:8000
```
...
5. 💡 后端服务需要单独启动（在 `E:\workcompany\agri-sub\agri-sub-backend` 目录）

## 插件使用流程

### 1. 构建插件（默认测试环境）
```bash
cd E:\bbb-test\easy-scraper-clone
npm run build
```
